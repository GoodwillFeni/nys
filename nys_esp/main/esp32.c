...
//#define NYS_API_URL "http://192.168.200.23:8000/api/device/message" //Work
...
static int64_t s_last_wifi_connect_attempt_us;
...
#define WIFI_RECONNECT_INTERVAL_US (10 * 60 * 1000000LL)  // 10 minutes
...
{
    // called by SNTP when the system time has been updated
    (void)tv;
...
static void time_maybe_start_sntp(void)
{
    ...
    esp_sntp_init();
    /* when SNTP eventually updates the system clock we'll be notified so
       that we can establish our internal time base; otherwise we only try
       once when wifi comes up and that is almost always too early. */
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        s_last_wifi_connect_attempt_us = esp_timer_get_time();
        esp_wifi_connect();
    } else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED) {
        xEventGroupClearBits(s_wifi_event_group, WIFI_CONNECTED_BIT);
        
        int64_t now_us = esp_timer_get_time();
        int64_t time_since_last_attempt_us = now_us - s_last_wifi_connect_attempt_us;
        
        /* Only attempt to reconnect if 10 minutes have passed since the last attempt */
        if (time_since_last_attempt_us >= WIFI_RECONNECT_INTERVAL_US) {
            ESP_LOGI(TAG, "WiFi disconnected, retrying connection...");
            s_last_wifi_connect_attempt_us = now_us;
            esp_wifi_connect();
        } else {
            int64_t wait_time_s = (WIFI_RECONNECT_INTERVAL_US - time_since_last_attempt_us) / 1000000LL;
            ESP_LOGI(TAG, "WiFi disconnected, will retry in %lld seconds", (long long)wait_time_s);
        }
        
        /* If AP is not yet running, start it so user can reconfigure if needed */
        if (!s_ap_running) {
            ap_start_portal();
        }
...
static void wifi_ensure_inited(bool need_sta, bool need_ap)
{
    ...
    s_last_wifi_connect_attempt_us = 0;
...
static const char *wifi_get_ip_str(void)
{
                ...
                if (nmea_parse_gga(work, &fix)) {
                    if (fix.has_fix) {
                        ...
                        if ((bits & WIFI_CONNECTED_BIT) != 0) {
                            ...
                            ESP_LOGI(TAG, "GPS FIX: lat=%.6f lon=%.6f sats=%d fixq=%d(%s) | WiFi: %s | http://%s/", fix.lat_deg, fix.lon_deg, fix.sats_used, fix.fix_quality, gps_fixq_to_str(fix.fix_quality), ip, ip);
                        } else {
...
static void queue_drain_step(void)
{
    ...
    /* if our stored message timestamp was zero and we now have a valid
       epoch from SNTP take the opportunity to stamp the record and save it
       so the server will see a reasonable time value. use the uptime delta
       to estimate when the message was actually queued so that old queued
       messages get different timestamps based on their relative ages. */
    if (best.queued_at_epoch_s == 0) {
        int64_t new_epoch = time_now_epoch_s();
        if (new_epoch > 0) {
            int64_t current_uptime_s = time_uptime_s();
            if (best.ts_s <= current_uptime_s) {
                int64_t age_delta_s = current_uptime_s - best.ts_s;
                best.queued_at_epoch_s = new_epoch - age_delta_s;
            } else {
                best.queued_at_epoch_s = new_epoch;
            }
            /* rewrite the blob while we still hold the mutex */
            char key[8];
