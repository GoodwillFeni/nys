  ...
  methods: {
    async getAccountDetails() {
      try {
        ...
        const response = await api.get(`/accounts/${this.$route.params.id}`, {
        ...
        // Navigate back if account not found
    ...
    async updateAccount() {
      ...
      try {
        ...
        const token = localStorage.getItem("token");
        const response = await api.put(`/accounts/${this.form.id}`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        const msg = error.response?.data?.message || "An error occurred while updating the account.";
        toast.error(msg);
      } finally {
        this.loading = false;
      }
